...
class PayAutorized : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_pay_autorized)
    ...
    fun confirm_Click(view: View) {
        ...
        if(card_number?.text.toString() == "" || month?.text.toString() == "" || cvv?.text.toString() == "" || year?.text.toString() == ""){
            ...
            if(new_balance >= 999999){
                ...
                new_balance = balance.toDouble()
                val intent = Intent(this ,MainMenu::class.java)
                intent.putExtra(MainMenu.BALANCE_AFTER_REFILL ,"$new_balance")
                intent.putExtra(MainMenu.NUMBER ,"$number")
                startActivity(intent)
            }
            else{
                MyDbManager.Refill("$new_balance" ,indexOfNumber + 1)
                val intent = Intent(this ,ConfirmAutorized::class.java)
                intent.putExtra(ConfirmAutorized.BALANCE_AFTER_REFILL ,"$new_balance")
                intent.putExtra(ConfirmAutorized.NUMBER ,"$number")
                startActivity(intent)
